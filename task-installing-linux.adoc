...
+
Un système Red Hat Enterprise Linux doit être enregistré auprès de Red Hat Subscription Management. S'il n'est pas enregistré, le système ne pourra pas accéder aux dépôts pour mettre à jour les logiciels tiers requis lors de l'installation.
...
. Sur la page *Définir la relation de synchronisation*, choisissez une source et une cible, puis sélectionnez *Continuer*.
+
Suivez les étapes jusqu'à atteindre la page *Data Broker Group*.

. Sur la page *Data Broker Group*, sélectionnez *Create Data Broker* puis sélectionnez *On-Prem Data Broker*.
+
...
. Saisissez un nom pour le courtier de données, puis sélectionnez *Continuer*.
+
Une fois la page d'instructions chargée, suivez les instructions qui incluent un lien unique pour télécharger le programme d'installation.

. Sur la page d'instructions :
+
.. Choisissez d'activer l'accès à *AWS*, *Google Cloud* ou aux deux.
