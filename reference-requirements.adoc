...
---
= Exigences relatives à la source et à la cible pour NetApp Copy and Sync
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Pour ajouter une clé KMS gérée par le client, vous devrez utiliser un courtier de données avec la link:task-installing-gcp.html#permissions-required-for-the-service-account["autorisations correctes"], et la clé doit se trouver dans la même région que le compartiment.
